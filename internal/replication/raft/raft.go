...
func (r *Raft) Apply(bytes []byte) error {
	...
	case *pb.Entry_Chunk:
		if c.Chunk.Offset == 0 {
			r.buff = make([]byte, c.Chunk.Total)
		}
		y.AssertTruef(r.buff != nil, "raft: missing initial chunk")
		copy(r.buff[c.Chunk.Offset:], c.Chunk.Data)
		if c.Chunk.Offset+uint64(len(c.Chunk.Data)) < c.Chunk.Total {
			return nil
		}
		defer func() {
			r.buff = nil
		...
		var tx pb.Tx
		if err := tx.UnmarshalVT(r.buff); err != nil {
			return err
		}
		start := time.Now()
