...
type ReplicationServiceClient interface {
	...
	Replicate(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ReplicateRequest, Ack], error)
...
func (c *replicationServiceClient) Replicate(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ReplicateRequest, Ack], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ReplicationService_ServiceDesc.Streams[1], ReplicationService_Replicate_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ReplicateRequest, Ack]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ReplicationService_ReplicateClient = grpc.BidiStreamingClient[ReplicateRequest, Ack]
...
type ReplicationServiceServer interface {
	...
	Replicate(grpc.BidiStreamingServer[ReplicateRequest, Ack]) error
...
func (UnimplementedReplicationServiceServer) Replicate(grpc.BidiStreamingServer[ReplicateRequest, Ack]) error {
...
func _ReplicationService_Replicate_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ReplicationServiceServer).Replicate(&grpc.GenericServerStream[ReplicateRequest, Ack]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ReplicationService_ReplicateServer = grpc.BidiStreamingServer[ReplicateRequest, Ack]
